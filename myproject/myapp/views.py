...
# from django.utils import FileWrapper
import mimetypes
import os
...
def displayplot(request):
    # inp = request.POST.get('param')
    
    # out = run([sys.executable, '//media//execution.py', inp], shell=False, stdout=PIPE)
    df = pd.read_csv(BASE_DIR, '/Assignment1-data.csv')
    ...
    for i in range(15, len(df)):
        m['TR14'].iloc[i:len(df)] = (m['TR14'].shift(1) - (m['TR14'].shift(1) / 14) + m['TR']).round(decimals = 2)
        dm["+DM14"].iloc[i:len(df)] = (dm["+DM14"].shift(1) - (dm['+DM14'].shift(1) / 14) + dm['+DM1']).round(decimals = 2)
        minus_dm['-DM14'].iloc[i:len(df)] = (minus_dm["-DM14"].shift(1) - (minus_dm['-DM14'].shift(1) / 14) + minus_dm['-DM1']).round(decimals = 2)
    ...
    # Renaming untitled column(time) to 'Time'
    ...
    return render(request, 'filedownload.html', {'data':data})
